...
)
{
	...
	for (int depth = 0; depth < diamension.depth; ++depth)
	{
		for (int row = 0; row < diamension.rows; ++row)
		{
			for (int col = 0; col < diamension.cols; ++col)
			{
				const double vel_abs = std::abs(velocity[depth][row][col]);
				const double area_tube = std::pow(radius[depth][row][col], 2) * declconst::PI;
				volume[depth][row][col] = vel_abs * area_tube * time_step;
			}
		}
	}
...
std::vector<std::vector<std::vector<int>>> func::Determine::gen_add_mnsc(const TMns& mnsc, const dst::Diamension& diamension)
{
	std::vector<std::vector<std::vector<int>>> add_mnsc = diamension.empty_table_templated<int>();

	for (int depth = 0; depth < diamension.node_depth(); ++ndepth)
	{
		for (int nrow = 0; nrow < diamension.node_rows(); ++nrow)
		{
			for (int ncol = 0; ncol < diamension.node_cols(nrow); ++ncol)
			{
				const std::vector<dst::Tube> cons
					= diamension.generate_tubes_connected_to_node(ndepth,nrow, ncol);
			...
			for (int direction = 0; direction < directions; ++direction)
			{
				...
				if (con.active)
				{
					const int end_type_fluid = mnsc[con.depth][con.row][con.col].type_near_node(direction);
			...
			for (int direction = 0; direction < directions; ++direction)
			{
				...
				{
					add_mnsc[con.depth][con.row][con.col] += cap_press_contb_from_direc(direction);
				}
			}
