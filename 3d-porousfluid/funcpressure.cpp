...
Tdouble func::Pressure::generate_equations_aug_matrix(
	...
	const TMns& mnsc,
	const std::vector<std::vector<std::vector<int>>>& add_mnsc,
	const dst::Diamension& diamension
)
{
	Tdouble equations_matrix = diamension.empty_aug_matrix();

	for (int depth = 0; depth < diamension.node_depth(); ++depth)
	{
		...
		for (int row = 0; row < diamension.node_rows(); ++row)
		{
			...
			{
				const int linear_node = diamension.linear_node_from_coordinate( depth, row, col);
				std::vector<double>& equation = equations_matrix[linear_node];

				const std::vector<dst::Tube> tubes_connected_vec =
					diamension.generate_tubes_connected_to_node(depth, row, col);
				...
				for (int direction = 0; direction < total_directions; ++direction)
				{
					...
					{
						const double r = radius[connection.depth][connection.row][connection.col];
						const dst::Mns& mns = mnsc[connection.depth][connection.row][connection.col];

						const int sign_of_capll_pressure
							= calc_sign_capll_pressure(
								mns.sign_of_capll_pressure(direction),
								add_mnsc[connection.depth][connection.row][connection.col],
								direction
							);
