...
)
{
	...
	for (int depth = 0; depth < diamension.depths; ++depth)
	{
		...
		for (int row = 0; row < diamension.rows; ++row)
		{
			...
			{
				const std::pair<int, int> linear_nodes_pair = diamension.linear_node_at_ends_of_tube(depth,row, col);
				...
				const double mu = mnsc[depth][row][col].mu(declconst::MU1, declconst::MU2);
				const double sign_of_capll_pressure = mnsc[depth][row][col].sign_of_capll_pressure(0) + add_mnsc[depth][row][col];
